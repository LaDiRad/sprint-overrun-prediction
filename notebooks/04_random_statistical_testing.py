# -*- coding: utf-8 -*-
"""04_random_statistical_testing.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1LXOgkw_wrqUvXKOSXn78FoBGwZuVyaTp
"""

# Statistical Significance Testing (Protocol A)
...
IN2, IN3 = f'{PROJECT}/02_random_benchmark', f'{PROJECT}/03_random_optimization'
OUT = f'{PROJECT}/04_random_statistical_testing'
...
mcc_final = mcc_per_fold(modele_opt[MODEL_FINAL], X_train, y_train, cv_skf,
                         thresholds[MODEL_FINAL])

print(f' MCC per fold — {RUNNER_UP} (optimized) ...')
mcc_runner = mcc_per_fold(modele_opt[RUNNER_UP], X_train, y_train, cv_skf,
...
# Wilcoxon for top 2 optimized models and for the first model optimized and non-optimized
...
mcc_final = mcc_per_fold(modele_opt[MODEL_FINAL], X_train, y_train, cv_skf, thresholds[MODEL_FINAL])
mcc_runner = mcc_per_fold(modele_opt[RUNNER_UP], X_train, y_train, cv_skf, thresholds[RUNNER_UP])
mcc_own_base = mcc_per_fold(rezultate_base[MODEL_FINAL]['model'], X_train, y_train, cv_skf, 0.5)
